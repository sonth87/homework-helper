...
// A keyword/regex allowlist for "is this actually a question" (the previous
// approach here) can never cover every greeting, aside, or bit of small talk
// across 13 UI languages plus whatever language the user types in — it only
// ever grows one missed phrase at a time. Handing the model an explicit
// escape hatch instead lets it judge content it's already reading anyway,
// in any language, with no maintenance burden. Shared by both the cloud
// path below and buildNanoPrompts() (storage.js) so the two prompt builders
// don't drift into wording separately.
export const CASUAL_CHAT_NOTE = 'If this is casual conversation, a greeting, or general chat unrelated to any question or exercise: ignore the structured format required above and just reply naturally and briefly, like an ordinary chat message.';
...
export function formatStudyPrompt(studyMode, prompt, outputLanguage = 'en') {
  ...
  if (!studyMode || studyMode === 'direct') {
    body = `[MODE: DIRECT ANSWER]\nIf the content below is an academic question or exercise: reply with ONLY the final answer, as CONCISELY as possible — no explanation, no analysis, no step-by-step breakdown. For multiple-choice questions, give only the option's letter/name (e.g. "C. NaN"). For math problems, give only the numeric result. No more than 2-3 sentences.\n${CASUAL_CHAT_NOTE}\n\nContent:\n${prompt}`;
  } else {
    switch (studyMode) {
      case 'hint':
        body = `[MODE: HINTS & SELF-STUDY GUIDANCE]\nIf the content below is an academic question or exercise: do NOT give the final answer right away — instead provide useful pedagogical hints, key formulas, and guiding questions so the student can work it out themselves.\n${CASUAL_CHAT_NOTE}\n\nContent:\n${prompt}`;
        break;
      case 'explain':
        body = `[MODE: IN-DEPTH EXPLANATION]\nIf the content below is an academic question or exercise: explain the underlying theory, related principles, and real-world intuition behind it in an easy-to-understand way — if it is a question, explain the knowledge it rests on rather than only answering it.\n${CASUAL_CHAT_NOTE}\n\nContent:\n${prompt}`;
        break;
      ...
      case 'step-by-step':
      default:
        body = `[MODE: DETAILED STEP-BY-STEP SOLUTION]\nIf the content below is an academic question, exercise, or homework problem: solve it with clear steps (Step 1, Step 2...), rigorous mathematical reasoning, LaTeX formulas ($...$), box the final answer, and select the correct option among the choices if any are given.\n${CASUAL_CHAT_NOTE}\n\nContent:\n${prompt}`;
    }
  }
