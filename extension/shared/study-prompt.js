...
- If the content is a SINGLE STANDALONE WORD (exactly 1 word, not a multi-word phrase), reply with exactly these 3 parts and nothing else:
  1. The original word followed by its IPA phonetic transcription in /.../ brackets.
  2. 1-2 example sentences using the word: each example gives the sentence in its original language plus its translation into ${targetLangName} (no need for pronunciation notes on the example sentences).
  3. A short description/definition of the word, written in ${targetLangName}.

- If the content is a PHRASE (2 or more words), a sentence, a paragraph, or longer text, reply with ONLY the accurate translation into ${targetLangName} and nothing else. Do NOT add IPA phonetics, do NOT add examples, no explanation, no analysis, no notes — no matter how short the term, proper noun, or phrase is.
